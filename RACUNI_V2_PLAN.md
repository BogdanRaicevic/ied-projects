...
- [ ] **Ticket 4.2.2:** When `tipPrimaoca === "firma"`: render `naziv`, `pib` (required), `maticniBroj` (required), `adresa`, `mesto`. `Controller` + MUI `TextField`.
- [ ] **Ticket 4.2.3:** When `tipPrimaoca === "fizicko"`: render `naziv` (required, placeholder "Ime i prezime"), `adresa` (required), `mesto`, `jmbg` (optional, help text "Unosi se za račune iznad zakonskog praga").
- [ ] **Ticket 4.2.4:** Switching `tipPrimaoca` prompts a confirm dialog if any field is dirty ("Izmena tipa primaoca će resetovati polja. Nastaviti?"), then resets the subset of fields that don't cross over (pib/maticniBroj/firma_id cleared going to `fizicko`; jmbg cleared going to `firma`).
- [ ] **Ticket 4.2.5:** Accepts prefill from navigation state. Seminari prefill always sets `tipPrimaoca = "firma"` and populates `firma_id` + firma fields. All fields remain editable after prefill.
